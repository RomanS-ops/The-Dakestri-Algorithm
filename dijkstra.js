...
let svg = document.getElementById('shortestPathSvg'),
...
function mouseClick (e) {
    ...
    let radWorkMode=document.getElementsByName("radWorkMode"),
        workMode = null,
        clickTarget = null;
    
    for(let i=0; i< radWorkMode.length; i++)
    ...
    switch (workMode) {
        ...
        case "setCostLabel":
            if(clickTarget.nodeName == "circle") {
				let vertex = clickTarget;
                showDialogVertexLabel(vertex);
            }

            if(clickTarget.nodeName == "line") {
				let edge = clickTarget;
...
function drawVertex(px, py){
    ...
    let vertex = document.createElementNS(svgns, "circle");
...
function drawEdge(vertex1, vertex2) {
    let x1 = vertex1.getAttributeNS(null, "cx"),
